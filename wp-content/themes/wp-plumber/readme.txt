=== WP Plumber ===
Tags: custom-menu, microformats, translation-ready, full-width-template, post-formats, one-column, two-columns, right-sidebar, custom-background, custom-colors, custom-logo
Requires at least: 4.0
Tested up to: 4.7.3
License: GNU General Public License v2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

WP Plumber WordPress Theme, Copyright 2017
WP Plumber is distributed under the terms of the GNU GPL

== Description ==
WP Plumber is another local business theme designed to help attract and convert customers in your local area. We designed this one with plumbers in mind, but really it could be used by any local business. With the ability to display your contact details front and center on both desktop and mobile, plus some nice features to promote your services on the home page it is a flexible, responsive and professional look for your business.

== Theme Setup ==
* For the top contact details, download Better Contact Details and add your info
* For Homepage Sections, create pages for eache sections and go to Appearance -> Customize -> Homepage Sections and Select a page to be assigned for each section.

== Changelog ==

= 1.0.2 =
* updated footer links with nofollow attributes

= 1.0.1 =
* added multilevel main menu support

= 1.0.0 =
* First public release


== Resources ==
*   TGM Plugin Activation library, Copyright Thomas Griffin, Gary Jones, Juliette Reinders Folmer, 
    License: GPL-2.0 or later license, 
    Source: http://tgmpluginactivation.com/#license

*   Fontawesome Icons Copyright Dave Gandy, 
    License: MIT License
    Source: https://github.com/FortAwesome/Font-Awesome and http://opensource.org/licenses/MIT

*   Bootstrap Copyright Mark Otto and jacob, 
    License: MIT License
    Source: https://github.com/twbs/bootstrap/blob/master/LICENSE

*   wp-bootstrap-navwalker by Emil Uzelac
    License: GNU General Public License v2.0
    Source: https://github.com/emiluzelac/wp-bootstrap-navwalker/blob/master/LICENSE.txt

*   Images used in screenshot.png, licensed under Creative Commons Zero (https://creativecommons.org/publicdomain/zero/1.0/deed.en):
    Copyright Wesley Caribe
    License: CC0 1.0 Universal (CC0 1.0)
    Source: https://unsplash.com/photos/TtN_obfWlGw

*   All the graphics bundled with this theme are created by the theme author, licensed under the GNU GPL.